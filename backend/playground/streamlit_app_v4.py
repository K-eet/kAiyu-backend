      ...
      if response.status_code == 200:
        ...
        if detect_res.status_code == 200:
          ...
          for idx, item in enumerate(detected_coords):
            ...
            # Get furniture image from backend
            furniture_detail = requests.get(f"{BACKEND_URL}/furniture/{item['furniture_id']}")
            if furniture_detail.status_code == 200:
              furniture_data = furniture_detail.json()
              if furniture_data.get("image_link"):
                st.image(
                  furniture_data["image_link"], 
                  caption=f"{furniture_data.get('name', 'Furniture')}", 
                  width=150)
