    ...
    if uploaded_file:
      ...
      if response.status_code == 200:
        ...
        detect_res = requests.post(
          f"{BACKEND_URL}/generated/object-detection?generated_room_id={generated_room_id}"
        )
        ...
        if detect_res.status_code == 200:
          ...
          # Plot bounding box + center dot + label on image
          ...
          for item in detected_coords:
            ...
            w, h = 50, 50  # Example box size for visualization only

            # Draw a rectangle around the center point
          ...
          st.warning(f"⚠️ Object detection failed: {detect_res.json().get('detail')}")
