...
class SimilarityService:
  ...
  def load_and_process_catalog(self, catalog_url: str):
    """Loads the product catalog from a CSV URL and computes image embeddings."""
    logger.info("Loading and processing product catalog...")
    try:
      # Load the catalog CSV into a pandas DataFrame
      df = pd.read_csv(catalog_url)
      # Rename columns for consistency
      df.columns = ['id', 'name', 'purchase_link', 'type', 'image_link']
      ...
      for _, row in df.iterrows():
        try:
          ...
          logger.error(f"Error fetching image for '{row['name']}' from URL {row['image_link']}: {e}")
      ...
      logger.error(f"Failed to load or process catalog from URL {catalog_url}: {e}")
  ...
  def find_similar_items(self, cropped_image: Image.Image, class_name: str, top_n: int = 1):
    ...
    # Preprocess the cropped image and extract its embedding
    input_tensor = self.preprocess(cropped_image)
    input_batch = input_tensor.unsqueeze(0).to(self.device)
    with torch.no_grad():
      query_embedding = self.feature_extractor(input_batch).squeeze().cpu().numpy()

    # Filter catalog for relevant items by category
    relevant_catalog_df = self.product_catalog_df[self.product_catalog_df['category'].str.lower() == class_name.lower()]
    
    if relevant_catalog_df.empty:
      logger.warning(f"No items found in catalog for category: {class_name}")
      return []

    # Get embeddings for the relevant items
...
# This ensures models and catalog are loaded only once at startup
