...
with tab1:
    ...
    # Show image comparison if available
    if (
        "original_image_path" in st.session_state
        and "generated_image_path" in st.session_state
    ):
        st.markdown("### 🖼️ Original vs Generated Image")
        image_comparison(
            img1=st.session_state["original_image_path"],
            img2=st.session_state["generated_image_path"],
            label1="Original",
            label2="Generated",
            width=700,
        )
...
with tab4:
    ...
    # Display furniture in 3-column layout
    if furniture_items:
