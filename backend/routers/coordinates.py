...
from backend.schemas.schemas import FurnitureCoordinatesModel
...
async def detect_save_and_find_similar(
  ...
  results = model(img_bgr, conf=0.4, iou=0.7, verbose=False)
...
# --- NEW ENDPOINT TO GET COORDINATES ---
@router.get("/coordinates/{generated_room_id}", response_model=List[FurnitureCoordinatesModel])
def get_coordinates_for_room(
  generated_room_id: str,
  db: Session = Depends(get_db)
):
  """
  Retrieves all saved furniture coordinates for a specific generated room.
  """
  coordinates = db.query(FurnitureCoordinates).filter(
    FurnitureCoordinates.generated_room_id == generated_room_id
  ).all()

  if not coordinates:
    raise HTTPException(
      status_code=404,
      detail=f"No coordinates found for room ID: {generated_room_id}"
    )
