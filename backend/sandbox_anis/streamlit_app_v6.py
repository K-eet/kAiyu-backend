...
with tab1:
  ...
  if st.button("✨ Generate Design & Find Similar Furniture", type="primary"):
    if uploaded_file:
      # This session state will hold our generated data
      ...
      with st.spinner("Generating new design... Please wait."):
        ...
        try:
          ...
          # Store the entire response in the session state
      ...
      st.header("3. Shop the Look")
      st.info("Click on a hotspot ✨ to see the recommended product.")

      with st.spinner("Detecting objects, saving coordinates, and finding similar items..."):
        try:
          # Read the generated image from its path to send to the backend
          with open(st.session_state.generated_data["generated_image_path"], "rb") as f:
            image_bytes = f.read()

          # *** THIS IS THE KEY CHANGE ***
          # We now pass the generated_room_id as a query parameter
          # to the endpoint that saves coordinates.
        ...
        if "detected_items" in similarity_results and similarity_results["detected_items"]:
          ...
          img_with_hotspots = cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR)
          ...
          if not matched_items:
            ...
            st.image(cv2.cvtColor(img_with_hotspots, cv2.COLOR_BGR2RGB), caption="Generated Image")
            st.stop()
          ...
          for item in matched_items:
            ...
            cv2.circle(img_with_hotspots, (center_x, center_y), 15, (255, 255, 255), -1)
            cv2.circle(img_with_hotspots, (center_x, center_y), 15, (0, 100, 255), 2)
            cv2.putText(
              img_with_hotspots, "✨", (center_x - 8, center_y + 8),
              cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 100, 255), 2
            )

          st.image(
            cv2.cvtColor(img_with_hotspots, cv2.COLOR_BGR2RGB),
            caption="Image with clickable hotspots",
...
# ... (Keep your code for tab2, tab3, and tab4 here)
# The other tabs (Gallery, Before vs. After, Catalog) remain the same.
with tab2:
...
#     with col2:
#         room_style = st.selectbox("Select Room Type:", ["Living Room", "Bedroom"])
#         design_style = st.selectbox("Select Design Style:", ["Scandinavian", "Modern", "Minimalist"])
